...
(created via `@acset_type MyType(MySchema, part_type=MarkAsDeleted)`).

# Example
```julia
ra = RuleApp(:add_v, my_rule, agent_shape)
sched = tryrule(ra)
abm_sched = ABMSchedule(:my_sched, sched, DiscreteHazard(1.))
abm = ABM([some_rule], schedules=[abm_sched])
```
"""
struct ABMSchedule
  name::Maybe{Symbol}
...
"""
An agent-based model, optionally with schedule-based events.
"""
@struct_hash_equal struct ABM
  ...
  function ABM(rules, dyn=[]; schedules::Vector{ABMSchedule}=ABMSchedule[]) 
    ...
    new(rules, dyn, names, schedules)
  end
end

additions(abm::ABM) = right.(abm.rules)

(F::Migrate)(abm::ABM) = ABM(F.(abm.rules), abm.dyn; schedules=abm.schedules)

Base.getindex(abm::ABM, i::Int) = abm.rules[i]
Base.getindex(abm::ABM, n::Symbol) = abm.rules[abm.names[n]]

Base.filter(f, abm::ABM) = ABM(filter(f, abm.rules); schedules=abm.schedules)
...
Base.copy(abm::ABM) = ABM(copy(abm.rules); schedules=copy(abm.schedules))
...
mutable struct RuntimeABM
  ...
  function RuntimeABM(abm::ABM, init::T; sampler=default_sampler) where T<:ACSet
    ...
    rt = new(init, init_homset.(abm.rules, Ref(init), Ref(additions(abm))), 
             ...
             mk_prob(abm, init)...)
    # Initialize the firing queue for rules
    ...
    # Initialize schedule timers (use negative indices to distinguish from rules)
    ...
    for (j, sched) in enumerate(abm.schedules)
      ...
      elseif sched.timer isa ClosureTime
        ...
        error("ABMSchedule timers must not depend on match state (use DiscreteHazard, ContinuousHazard, or ClosureTime)")
      end
...
function run!(abm::ABM, rt::RuntimeABM, output::Traj;
  ...
  while rt.nevent < maxevent && rt.tnow < maxtime
    ...
    if !isempty(abm.dyn) && dt < new_time 
      ...
      # Execute schedule events first (they do a full state replacement + refresh)
      ...
      for (event, _key) in schedule_events
        ...
        # Re-enable this schedule's timer for next firing
      ...
      # If any schedule fired, rebuild all rule hom-sets from scratch
      if schedule_fired
        for (i, (ruleᵢ, clocksᵢ)) in enumerate(zip(abm.rules, rt.clocks))
          clocksᵢ isa ExplicitHomSet || continue
          # Disable all existing clocks for this rule
        ...
        # Skip normal rule execution for this timestep
